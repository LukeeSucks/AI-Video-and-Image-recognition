...
# ObjectTracker class for tracking objects across frames
class ObjectTracker:
        ...
        if not self.tracked_objects:
            ...
            if detection_centroids:
                ...
                for (row, col) in zip(rows, cols):
                    ...
                    obj.centroid = detection_centroids[col]
...
def add_glow_effect(image, mask, ksize=(0, 0), sigma=5):  # Reduced sigma for less blurring
    glow = cv2.GaussianBlur(mask, ksize, sigma)
    return cv2.addWeighted(image, 1.0, glow, 0.3, 0)  # Adjusted blending for better clarity

# Function to draw futuristic bounding boxes
def draw_futuristic_box(image, x, y, w, h, color, thickness):
    # Create corner lines
    ...
    mask = cv2.rectangle(mask, (x, y), (x + w, y + h), color, thickness)
...
def process_frame(frame: np.ndarray, tracker: ObjectTracker, net: cv2.dnn_Net,
                  ...
                  class_ids_to_detect: List[int]) -> np.ndarray:
    (H, W) = frame.shape[:2]
    ...
    reference_dimension = 720  # Reference resolution (you can adjust this value)
    ...
    for obj in tracked_objects:
        if obj.annotate:
            ...
            # Draw the futuristic bounding box
            overlay = draw_futuristic_box(overlay, x, y, w, h, base_color, bbox_thickness)
    ...
    alpha = 1.0  # Full opacity, avoiding excessive transparency
...
def main():
    ...
    if is_image(input_path):
        ...
        annotated_image = process_frame(image, tracker, net, classes, config, random_colors, class_ids_to_detect)
        ...
        try:
            with tqdm(total=total_frames, desc='Processing Video', unit='frame') as pbar:
                while True:
                    ...
                    annotated_frame = process_frame(frame, tracker, net, classes, config, random_colors, class_ids_to_detect)
